...
public class ShadowCaster {
  ...
  public static Quad castQuad(
    ...
    for (int i = 0; i < vertices.length; i++) {
      ...
      if (currentPosition.y == 0) {
        projectedVertices[i] = new Vertex(vertex.transform(), vertex.localPosition(), 0);
      } else {
        ...
        projectedVertices[i] = new Vertex(vertex.transform(), projectedPosition, 0);
      }
    }
  ...
  public static ShadowEllipse castSphere(Vertex center, float radius, Transform transform, GlobalLight light) {
    PVector lightDirection = light
        .getDirectionVector(PApplet.radians(15))
        .normalize();
    ...
    PVector projectedPosition = PVector.sub(
        ...
        PVector.mult(lightDirection, center.height() * lightDirection.y));

    return new ShadowEllipse(transform, radius, projectedPosition, new float[] { 80, 80, 80, 80 });
  }
}
